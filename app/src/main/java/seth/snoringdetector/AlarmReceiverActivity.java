...
public class AlarmReceiverActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (AlarmStaticVariable.level == 0) {
            Vibrator vibrator = (Vibrator) getSystemService(VIBRATOR_SERVICE);
            vibrator.vibrate(AlarmStaticVariable.partten, -1);
    ...
    private Runnable updateTimer = new Runnable() {
        public void run() {
            ...
            while (true) {
                ...
                if (currentTime - startTime > AlarmStaticVariable.level)
                    break;
            }
            mMediaPlayer.stop();
            AlarmStaticVariable.inProcess = false;
            AlarmReceiverActivity.this.finish();
        }
    };
    ...
    private void playSound(Context context, Uri alert) {
        ...
        try {
            ...
            if (audioManager.getStreamVolume(AudioManager.STREAM_ALARM) != 0) {
                ...
                mMediaPlayer.setVolume(AlarmStaticVariable.lvolumn,
                        AlarmStaticVariable.rvolumn);
