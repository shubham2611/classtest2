        ...
        while (true) {
            if (draw_isRecording) {
                ...
                synchronized (AlarmStaticVariable.inBuf) {
                    if (AlarmStaticVariable.inBuf.size() == 0)
                        continue;
                    buf = (ArrayList<short[]>) AlarmStaticVariable.inBuf
                            .clone();// save
                    AlarmStaticVariable.inBuf.clear();// clear
                }
                for (int i = 0; i < buf.size(); i++) {
                    short[] tmpBuf = buf.get(i);
                    SimpleDraw(X_index, tmpBuf, AlarmStaticVariable.rateY,
